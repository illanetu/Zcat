...
import { ROBOTO_ITALIC_BASE64 } from './roboto-font-italic-base64'
...
const FONT_ITALIC_VFS_NAME = 'Roboto-Italic.ttf'
...
/**
 * Регистрирует Roboto Italic в документе jsPDF (если base64 не пустой)
 * @param {import('jspdf').jsPDF} doc
 */
function setCyrillicFontItalic(doc) {
  if (!ROBOTO_ITALIC_BASE64 || ROBOTO_ITALIC_BASE64.length === 0) return
  doc.addFileToVFS(FONT_ITALIC_VFS_NAME, ROBOTO_ITALIC_BASE64)
  doc.addFont(FONT_ITALIC_VFS_NAME, FONT_ID, 'italic', undefined, 'Identity-H')
}
...
export async function buildArtworkPdf(imageDataUrl, label, description, descriptionTitle = 'Описание') {
  ...
  if (label && label.title) {
    ...
    if (label.techniqueAndMaterial) {
      if (ROBOTO_ITALIC_BASE64 && ROBOTO_ITALIC_BASE64.length > 0) {
        doc.setFont(FONT_ID, 'italic')
      }
...
function getSafePdfFilename(filename) {
  let name = filename && filename.trim() ? filename.replace(/[<>:"/\\|?*]/g, '_') : 'artwork'
  name = name.replace(/\s*\(\d+\)\s*$/, '').trim()
  if (name.toLowerCase().endsWith('.pdf')) name = name.slice(0, -4).trim()
  return name ? `${name}.pdf` : 'artwork.pdf'
}
...
export function downloadPdf(blob, filename) {
  ...
  if (isMobileDevice()) {
    // На мобильных компонент хранит blob и по нажатию кнопки вызывает Web Share API.
