...
// Превью слева: 50% от прежнего размера (45×60), сохраняем пропорции
...
export async function buildArtworkPdf(imageDataUrl, label, description, descriptionTitle = 'Описание') {
  ...
  // 2. Этикетка справа — в том же виде, что и на форме (автор, название, техника, размер, год)
  if (label && label.title) {
    ...
    const cleanTitle = String(label.title).replace(/^[\s«""]+|[\s»""]+$/g, '').trim()
    doc.text(`«${cleanTitle}»`, labelLeft, labelY)
