 ...
 * @returns {Promise<{ description: string, descriptionPoetic: string }>} - Два варианта описания
 */
export async function generateDescription(imageBase64, artworkData, locale = 'ru') {
  try {
    // Удаляем префикс data:image/...;base64, если он есть
    const base64Data = imageBase64.includes(',') 
      ? imageBase64.split(',')[1] 
    ...
    return {
      ...
      descriptionPoetic: data.descriptionPoetic || ''
    }
  } catch (error) {
