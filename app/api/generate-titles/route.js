    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Ты эксперт по искусству. Твоя задача - предложить краткие, выразительные названия для произведений искусства на основе изображения. Названия должны быть лаконичными (1-2 слова), отражать суть произведения и быть подходящими для каталогов галерей. Отвечай только списком названий, по одному на строку, без нумерации и дополнительных комментариев.'
          },
          {
            role: 'user',
            content: [
              {
                type: 'text',
                text: 'Проанализируй это изображение произведения искусства и предложи 5 кратких названий (по 1-2 слова каждое), которые подходят для каталога галереи. Названия должны быть выразительными и отражать суть произведения.'
              },
              {
                type: 'image_url',
                image_url: {
